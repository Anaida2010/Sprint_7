...
class CourierApiHelper:
    ...
    def delete_courier_by_id(courier_id):
        return requests.delete(Urls.COURIER_HANDLE + '/' + str(courier_id))

    @staticmethod
    @allure.step('Удаление курьера с помощью логина и пароля.')
    def delete_courier_by_login_password(login, password):
        response = requests.post(Urls.COURIER_LOGIN_HANDLE, json={"login": str(login), "password": str(password)})
...
class OrderApiHelper:
    ...
    def accept_order(track_order, courier_id):
        return requests.put(Urls.ORDERS_ACCEPT_HANDLE + '/' + str(track_order) + '?courierId=' + str(courier_id))
    ...
    def request_order_by_track(track):
        return requests.get(Urls.ORDERS_GET_BY_TRACK_HANDLE + '/?t=' + str(track))
